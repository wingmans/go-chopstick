...
func TestServerNormalizesCIKPadding(t *testing.T) {
	...
	filing := &edgar.ParsedFiling{
		...
		Metadata: edgar.FilingMetadata{
			...
			Filers: []edgar.SubmissionFiler{{CIK: "789019", Name: "Example Corp."}},
		},
	...
	for _, cik := range []string{"789019", "0000789019"} {
		...
		request := httptest.NewRequest(http.MethodGet, "/api/filings?cik="+cik, nil)
	...
	record := httptest.NewRecorder()
	server.ServeHTTP(record, httptest.NewRequest(http.MethodGet, "/filings/789019/0001193125-26-027207", nil))
